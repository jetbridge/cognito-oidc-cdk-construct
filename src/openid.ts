...
export class OpenID {
  ...
  getJwks() {
    return getSecrets().then((secrets: any) => {
      const pubKey = secrets.JWT_KEY_PUB
      return { keys: [crypto.getPublicKey(pubKey)] }
    })
  }
  ...
  getAuthorizeUrl(client_id: any, scope: any, state: any, response_type: any) {
    return getSecrets().then((secrets) => this.service(secrets).getAuthorizeUrl(client_id, scope, state, response_type))
  }
